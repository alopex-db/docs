...
### v0.x（現行バージョン）— TTY では TUI がデフォルト
- **デフォルト動作**: TTY では TUI、非 TTY ではバッチモード。
- **目的**: 基盤となる CLI 機能の安定化と API 互換性の確立。
- **含まれる機能**:
  - Phase 1〜3 の CRUD 操作、接続管理、認証。
  - 出力フォーマット（table/JSON/CSV/TSV）。
  - プロファイル管理、オブザーバビリティ機能。
- **TUI の位置づけ**: TTY で自動起動（プレビュー機能）。バッチへは `--batch` / `--output` で明示的に切替。

### v1.0（次期メジャーバージョン）— TUI がデフォルト
- **デフォルト動作**: TTY では TUI モード（インタラクティブ）。
- **目的**: ユーザー体験の向上と探索的データ分析のサポート。
- **移行方針**:
  - バッチモードは `--batch` / `-b` / `--output` で引き続き利用可能。
...
| `alopex sql --tui` | **廃止**（TTY では `alopex sql` が TUI を起動） |
