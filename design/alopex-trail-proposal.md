# alopex-trail 設計 — late-bound schema のログ/イベントストア

**バージョン**: 2.0
**作成日**: 2026-07-29
**ステータス**: 設計中（着手可能。実装はこれから）
...
Alopex には既に 2 つのストアがある。**分ける基準は「書き込んだ行をその後どうするか」である。**

| 製品 | 書き込みの性質 | データの性質 | 形の制約 |
|---|---|---|---|
| **Alopex DB** | トランザクショナル（ACID、更新・削除、MVCC） | **状態**。意図して変更する | スキーマを宣言する |
| **alopex-skulk** | 追記専用 | 観測値。周期的に到着する | measurement + tags で固定、数値中心 |
| **alopex-trail** | 追記専用 | **事象の記録**。後から変わらない | 制約なし |

ログを更新する者はいない。起きたことを記録したものであり、起きたことは変わらないためである。したがって**トランザクショナルなストアが一貫性のために払うコストは、ログに対しては何も買わない**。一方でその型の厳格さは、デプロイのたびにマイグレーションを要求する。

skulk は追記専用という点で trail と同じだが、**周期的に到着する数値系列**のために作られている。series identity が安定していなければならず、時刻も必須である。

**追記専用でありながら形に制約を課さない** — これが trail の領域である。

skulk と trail は追記専用・列指向・Parquet 永続という同じストレージ基盤を共有し、上位のデータモデルだけが分かれる。
...
- 性能目標の確定値（第 11 章で未確定として扱う）
- クレート名・API シグネチャ等の実装詳細
- 内部クエリ言語の具体的な構文（§11.4）

**着手可能な状態である。** v0.1 の範囲（§10）は依存が揃っており、実装を開始できる。ただし WAL と取り込み層は skulk v0.3.1 の完了後に取り込む（§11.2）。
