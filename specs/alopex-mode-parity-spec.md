...
> **対象バージョン**: Alopex DB v0.6（alopex-core / alopex-sql / alopex-embedded / alopex-server / alopex-cli v0.6 系）
> **ステータス**: ドラフト
...
| SF-CLUSTER | クラスタ | v0.7 以降で定義（予約） | データディレクトリ |
...
| 5 | SF-CLUSTER | **SKIP 表示**（「v0.7 で有効化」と明示） | なし |

- 第 5 幕は実装しない。スクリプトは幕の存在と SKIP 理由を表示する（スキップを完了と偽らない）。
...
| writer \ reader | 組み込み API | CLI | HTTP | gRPC |
|----------------|:---:|:---:|:---:|:---:|
| 組み込み API | ✓ | ✓ | ✓ | ✓ |
| CLI | ✓ | ✓ | ✓ | ✓ |
| サーバー（HTTP 経由の DML） | ✓ | ✓ | ✓ | ✓ |
...
- S1 第 5 幕・S2 マトリクスの「SF-CLUSTER」列は本仕様の時点で**予約**とし、v0.7 の cluster-aware リリース（query router / membership）と同時に有効化する。
- 有効化時の追加要件は「クラスタノードが既存データディレクトリを取り込み、同一コーパス・同一検証クエリが一致すること」であり、コーパス・期待値・正規化規則は本仕様のものを変更なく用いる。
