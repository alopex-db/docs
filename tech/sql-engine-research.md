...
| 2026-06-27 | ランタイム性能比較（セクション 10.6）追加。パーサー用途での Nim 優位性を分析 |
...
### 10.6 ランタイム性能の比較（生成アーティファクト）

パーサー実装言語を Nim や Roc に移行した場合、**生成されるバイナリの実行性能**に影響があるかを評価する。

#### 汎用ベンチマーク結果

複数のベンチマークスイート（[programming-language-benchmarks](https://programming-language-benchmarks.vercel.app/)、[kostya/benchmarks](https://github.com/kostya/benchmarks)、[benchLANG](https://github.com/GlebOlsen/benchLANG)）の 2025〜2026 年データを集約した。

| ベンチマーク | **Nim** | **Rust** | 差 | ワークロード特性 |
|---|---|---|---|---|
| Binary Trees (18) | **685 ms** | 1,259 ms | **Nim 46% 高速** | メモリアロケーション主体 |
| Brainfuck (bench.b) | 1,171 ms | **1,008 ms** | Rust 14% 高速 | インタプリタ実行 |
| Base64 | 1,348 ms | **842 ms** | Rust 38% 高速 | バイト列処理 |
| Fasta (2.5M) | 209 ms | **88 ms** | Rust 2.4x 高速 | 文字列生成 |
| Mandelbrot (5000) | 288 ms | **246 ms** | Rust 17% 高速 | 数値計算 |
| N-Body (5M) | 319 ms | **163 ms** | Rust 2x 高速 | 浮動小数点演算 |
| Regex Redux (2.5M) | 1,635 ms | **438 ms** | Rust 3.7x 高速 | 正規表現 |
| Spectral Norm (8000) | 3,589 ms | **492 ms** | Rust 7.3x 高速 | 行列演算（マルチスレッド） |

**ソース**: programming-language-benchmarks.vercel.app (2025-08、AMD EPYC 7763、nim 2.2.4 / rustc 1.88.0)

**Roc**: 比較ベンチマークは未公開。公式は「GC 付き主要言語（Go, C#, Java, JS）より速い」ことを目標に掲げるが、Rust・Nim との性能比較データは存在しない。

#### パーサーのワークロード特性と Nim の優位性

汎用ベンチマークでは Rust が大半で優位だが、**SQL パーサーのワークロードは汎用ベンチマークとは本質的に異なる**。

SQL パーサーの処理は以下で構成される:

1. **大量の AST ノード生成**（ヒープアロケーション主体）
2. **文字列比較**（キーワード判定）
3. **再帰的なツリー構築**（パターンマッチ + ノード接続）

この特性は Binary Trees ベンチマークに最も近い。Binary Trees は**ツリーノードの大量生成・走査・解放**を計測するワークロードであり、SQL パーサーの AST 構築と本質的に同じ処理パターンである。

| ワークロード特性 | SQL パーサーとの関連度 | Nim vs Rust |
|---|---|---|
| **ツリーノード大量生成** (Binary Trees) | ◎ 直接対応 | **Nim 46% 高速** |
| 数値計算 (N-Body, Mandelbrot) | × 無関係 | Rust 優位 |
| 正規表現 (Regex Redux) | △ レキサーで一部使用 | Rust 優位 |
| マルチスレッド (Spectral Norm) | × パーサーはシングルスレッド | Rust 優位 |

**Nim が Binary Trees で 46% 高速な理由**:

Nim の ORC（Ownership-based Reference Counting）は、大量の小さなオブジェクトを生成・破棄するパターンで Rust の Box + Drop よりも効率的に動作する。Rust は `Box<Expr>` によるヒープアロケーションのたびに `alloc::alloc` → `alloc::dealloc` のペアが発生するが、Nim の ORC はサイクルコレクターと組み合わせたバッチ解放により、このオーバーヘッドを削減する。

#### パーサー性能への影響の結論

| 観点 | 評価 |
|---|---|
| AST 構築性能 | **Nim が Rust より高速**（Binary Trees で 46% 高速） |
| レキサー性能 | 同等（文字列比較は両言語で差異小） |
| FFI オーバーヘッド | 無視可能（パース呼び出しは 1 回/クエリ、ナノ秒オーダー） |
| 全体への影響 | パーサーは SQL 実行全体の数%以下。ボトルネックはストレージ I/O |

**結論: Nim でのパーサー実装は性能劣化ではなく、むしろ性能向上に寄与する可能性が高い。** SQL パーサーの主要ワークロード（AST ノードの大量生成・走査）は Nim の ORC メモリ管理が Rust の Box パターンより効率的に処理する領域である。Rust が優位な数値計算・正規表現・マルチスレッドはパーサー層では使用されないため、Nim への移行によるデメリットはない。加えて、ビルド時間が 63 倍高速化される開発効率の改善は、性能上の微差を遥かに上回る。

### 10.7 Alopex SQL パーサーへの適用可能性
