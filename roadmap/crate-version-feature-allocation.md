...
| 5 | 汎用スカラー関数 + 関数レジストリ基盤 | alopex-sql | **ws v0.7.3** | なし(追加のみ) | レジストリ導入、v0.5.3 カタログの数値/文字列/条件/型関数が動作 |
| 6a | ハッシュ/UUID/エンコード関数 | alopex-sql | **ws v0.7.4** | なし | SHA256/MD5/UUIDV7/HEX/ENCODE 等が動作。**着手前に spec 化必須** |
| 6b | システム関数 + PRAGMA (memory_stats 等) | alopex-sql, alopex-core (計測) | **ws v0.7.5** | なし | memory_stats/io_stats/clear_cache、PRAGMA。**spec 化必須** |
...
- **v0.7.0 でやるべきだった機能（#3/#4/#5/#6）は、すべて v0.7.x パッチ系列で完済する。** これらは「v0.7 の実装漏れ」であり、新バージョンの予定を消費させない。
...
- #5/#6 は系統A（単一ノード・Chirps 非依存）。#3 と独立に並行可能だが、**関数レジストリ(#5)を先に入れてから #6 を載せる**（レジストリ無しで #6 を足すと巨大 match が再び肥大）。
- #6a/#6b は現在 **spec が存在しない**（ロードマップ記載のみ）。着手前に requirements/design/tasks を起こすことを DoD の前提とする。
...
| **v0.7.3** | **#5 汎用スカラー関数 + レジストリ基盤** | alopex-sql | 🔴 債務 |
| **v0.7.4** | **#6a ハッシュ/UUID/エンコード関数** | alopex-sql | 🔴 債務(要spec) |
| **v0.7.5** | **#6b システム関数 + PRAGMA** | alopex-sql + alopex-core | 🔴 債務(要spec) |
...
> **v0.7.0 の実装漏れ（#3-#6）は v0.7.x パッチ系列で完済する。v0.8.0 に繰り越さない。** v0.7.1 は依存/セキュリティ（RUSTSEC-2026-0176/0177/0104/0099/0098、pyo3 0.24→0.29 / rustls 0.21→0.23）で埋まっているため、機能債務は v0.7.2 以降に順次配置。**v0.8.0 は本来の「分散クエリ本実装」予定を温存**する。
...
```
債務完済 (v0.7.x パッチ系列):
  v0.7.0 ✅ ─► v0.7.1 🔧(依存/セキュリティ) ─► [v0.7.2 #3/#4 +B-1] ─► [v0.7.3 #5] ─► [v0.7.4 #6a] ─► [v0.7.5 #6b]
                                                                                                          │
本来予定 (温存・繰り越さない):                                                                            ▼
...
- **その後: v0.7.3（#5）→ v0.7.4（#6a）→ v0.7.5（#6b）** と、v0.7.0 の実装漏れを 1 パッチ 1 機能で順に完済する。
